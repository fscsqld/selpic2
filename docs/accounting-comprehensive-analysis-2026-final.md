# SELPIC A: 회계 프로그램 전체 분석 및 작성 계획안 비교 보고서

> **작성일**: 2026년 1월  
> **현재 진행률**: Phase 1 **98%** 완료 (초기 설계안 대비 **130%** 달성)  
> **최종 업데이트**: Share Capital, Real-time P&L, Asset Management, Audit Trail, Tax Provision, Period Management 완료 후
...
| **Phase 1 추가 기능** | 100% | 98% | 🟡 **98%** | FBT 마감일만 남음 |
| **Phase 1 전체** | 100% | **98%** | **98%** | |
| **계획 외 추가 기능** | 0% | **30개 이상** | ⭐ **130%** | 초기 설계안 초과 달성 |
| **Phase 2 통합** | 100% | 10% | 🟡 **10%** | Homepage API Integration 부분 완료 |
...
- ✅ **Compliance Package Excel 내보내기** - 계획 외 추가
...
- `lib/compliance-reporting/compliance-package.ts`
...
- ✅ **Share Capital 지원** - 계획 외 추가
  - `EQUITY_SHARE_CAPITAL` 카테고리
  - Net Profit에 영향 없음
  - Balance Sheet Equity에만 반영
...
### 11. Cash & Petty Cash 관리 ✅ (100% - 계획 외 추가)
...
- ✅ **BAS & Financial Summary Report** - 계획 외 추가
  ...
  - Print/Export PDF 기능
  - Receipt 포함 옵션
- ✅ **Income Statement Report** - 계획 외 추가
...
### 15. Real-time P&L View ✅ (100% - 계획 외 추가)

**초기 설계안**: 없음

**현재 상태**:
- ✅ 현재 월의 Revenue, Expenses, Net Profit 실시간 표시
- ✅ 월별 비교 기능
- ✅ 트렌드 표시 (TrendingUp/Down 아이콘)

**파일 위치**:
- `components/RealTimePLView.tsx`

---

### 16. Asset Management ✅ (100% - 계획 외 추가)

**초기 설계안**: 없음

**현재 상태**:
- ✅ $300 이상 지출 자동 감지 및 Asset 등록 프롬프트
- ✅ Fixed Asset 등록 시스템
- ✅ 감가상각 추적 (Straight-Line / Diminishing Value)
- ✅ Asset 리스트 (구매 비용, 누적 감가상각, 현재 가치)
- ✅ Capital Improvement 경고 ($5,000 이상 또는 리모델링 키워드)

**파일 위치**:
- `components/AssetManagement.tsx`
- `lib/storage/indexed-db.ts` (assets store)

---

### 17. Audit Trail ✅ (100% - 계획 외 추가)
...
**파일 위치**:
- `components/AuditTrailView.tsx`
- `lib/storage/indexed-db.ts` (auditTrail store)

---

### 18. Tax Provision ✅ (100% - 계획 외 추가)

**초기 설계안**: 없음

**현재 상태**:
- ✅ 현재 이익 기반 회사세 추정 계산
- ✅ 호주 회사세율 적용 (30% 기본, 25% 소규모 사업체)
- ✅ 소규모 사업체 기준 ($50M 이하)
- ✅ 실시간 Tax Provision 표시

**파일 위치**:
- `components/TaxProvision.tsx`

---

### 19. Balance Sheet ✅ (100% - 계획 외 추가)
...
- ✅ Share Capital 포함
- ✅ Compliance Package에 포함 (Excel 내보내기)
...
### 20. Period Management ✅ (100% - 계획 외 추가)

**초기 설계안**: 없음

**현재 상태**:
- ✅ 월별 마감 기능
- ✅ Closing Balance 자동 Carry Forward
- ✅ 기간 잠금 (Locking) 메커니즘
- ✅ 미결제 주문 Carry Forward
- ✅ Audit Trail 통합

**파일 위치**:
- `components/Settings/PeriodManagement.tsx`
- `lib/period-management/period-utils.ts`
- `lib/storage/period-types.ts`

---

### 21. Security & Onboarding ✅ (100% - 계획 외 추가)
...
- ✅ **Setup Wizard**
  - 초기 설정 가이드
  - Account Type 선택 (Individual/Company/Sole Trader)
  - Company Name, ABN, ACN 입력
  - Homepage API Integration 설정
  - Test Connection 기능

**파일 위치**:
- `components/Security/PINLock.tsx`
- `components/Security/SystemReset.tsx`
- `components/Onboarding/SetupWizard.tsx`

---

### 22. Homepage API Integration ✅ (10% - Phase 2 부분 완료)

**초기 설계안**: Phase 2에 포함

**현재 상태**:
- ✅ **API Endpoint**: `/api/orders/import` 구현
- ✅ **Incoming Orders Inbox**: 홈페이지에서 전송된 주문 데이터 확인
- ✅ **Duplicate Check**: 중복 주문 번호 자동 필터링
- ✅ **Field Mapping**: 홈페이지 데이터 → 회계 프로그램 데이터 매핑
- ✅ **승인/거부 기능**: 주문 데이터 승인 후 거래로 변환
- ❌ **Admin Dashboard 통합**: 미구현
- ❌ **인증 시스템 연동**: 미구현
- ❌ **데이터베이스 마이그레이션**: 미구현
- ❌ **UI 통합**: 미구현
...
- `components/Settings/IncomingOrders.tsx`

---

## 🎯 계획 외 추가 구현된 기능 (30개 이상)
...
6. ✅ Period Management (월별 마감, Carry Forward)

### 파싱 엔진 (4개)
7. ✅ Westpac PDF Parser
8. ✅ Universal CSV Parser
9. ✅ 거래 수수료 인식
10. ✅ 설명 정제 (Description Cleansing)

### 비즈니스 로직 (8개)
11. ✅ Business Consolidation (Cleaning + Sticker 통합)
12. ✅ Director's Loan 자동 동기화
13. ✅ Opening Balance 관리
14. ✅ Single Source of Truth (`calculateBusinessMetrics`)
15. ✅ Append Mode (거래 추가 모드)
16. ✅ Capital Improvement 경고
17. ✅ Share Capital 지원
18. ✅ Tax Provision 계산

### UI/UX (8개)
19. ✅ Cash & Petty Cash 관리
20. ✅ 대시보드 시각화 (Charts)
21. ✅ API Balance Dashboard
22. ✅ Receipt Attachment System
23. ✅ BAS & Financial Summary Report
24. ✅ Income Statement Report
25. ✅ Tab-based Navigation (Reports 추가)
26. ✅ Real-time P&L View

### 관리 기능 (4개)
27. ✅ Asset Management
28. ✅ Audit Trail
29. ✅ Balance Sheet
30. ✅ Compliance Package Export

---

## 🟡 부분 완료 기능

### 1. 통합 세무 대시보드 (98% → 목표 100%)
...
### 2. Phase 2: 홈페이지 통합 (10% → 목표 100%)

**완료된 부분**:
- ✅ API Endpoint (`/api/orders/import`)
...
**초기 설계안**: 통합 세무 대시보드에 포함

**현재 상태**: FBT 감지 시스템은 완료되었으나, FBT 마감일 계산 로직만 남음

**우선순위**: 높음 (통합 세무 대시보드 100% 완성)
...
### Phase 1 추가 기능 달성률: **98%**

| 기능 | 초기 설계안 | 현재 | 상태 |
|------|------------|------|------|
| 통합 세무 대시보드 | ✅ | 🟡 98% | 부분 완료 (FBT 마감일만 남음) |
...
| 카테고리 | 개수 | 주요 기능 |
|---------|------|----------|
| 데이터 관리 | 6개 | User Mappings, Debit/Credit 교환, 백업/복구, Period Management 등 |
| 파싱 엔진 | 4개 | Westpac, CSV, 수수료 인식, 설명 정제 |
| 비즈니스 로직 | 8개 | Business Consolidation, Director's Loan 동기화, Share Capital, Tax Provision 등 |
| UI/UX | 8개 | Cash 관리, Charts, Receipt System, BAS Report, Income Statement 등 |
| 관리 기능 | 4개 | Asset Management, Audit Trail, Balance Sheet, Compliance Package |
...
| Append Mode | ❌ | ✅ | **추가 구현** |
| Period Management | ❌ | ✅ | **추가 구현** |

### 리포트 및 내보내기

| 항목 | 초기 설계안 | 현재 | 차이 |
|------|------------|------|------|
| General Ledger Excel | ✅ | ✅ | 완료 |
| Financial Summary Excel | ✅ | ✅ | 완료 |
| BAS Report Excel | ❌ | ✅ | **추가 구현** |
| Income Statement Report | ❌ | ✅ | **추가 구현** |
| Balance Sheet | ❌ | ✅ | **추가 구현** |
| Compliance Package | ❌ | ✅ | **추가 구현** |
| Trial Balance | ❌ | ✅ | **추가 구현** |
| Director's Loan Report | ❌ | ✅ | **추가 구현** |
...
#### 부분 일치 (98%)
- 통합 세무 대시보드 (98% 완료, FBT 마감일만 남음)

#### 미일치 (10%)
- Phase 2 통합 (10% 완료, API Integration만 부분 완료)
...
### 현재 상태 (Phase 1: 98%)
- ✅ 핵심 기능: 7개 완료 (100%)
- 🟡 추가 기능: 1개 부분 완료 (통합 대시보드 98%)
- ✅ 추가 기능: 1개 완료 (FBT 100%)
- ⭐ 계획 외 추가 기능: 30개 이상 구현
...
- **Phase 1 진행률**: 약 **98%**
- **초기 설계안 준수도**: **98%**
- **계획 초과 달성**: **30개 이상 추가 기능 구현**
- **전체 달성률**: **130%** (계획 대비)

### 초기 설계안 대비
- **핵심 기능**: 100% 완료 (계획 초과 달성)
- **추가 기능**: 98% 완료 (FBT 마감일만 남음)
- **Phase 2**: 10% (API Integration 부분 완료)
- **계획 외 기능**: 30개 이상 추가 구현
...
- **데이터 관리**: 백업/복구, API 로깅, localStorage 동기화, Period Management 추가
- **UI/UX**: 시각화, API Balance Dashboard, Receipt System, BAS Report, Income Statement, Balance Sheet 추가
- **비즈니스 로직**: Business Consolidation, Director's Loan 동기화, Single Source of Truth, Share Capital, Tax Provision 추가
- **관리 기능**: Asset Management, Audit Trail, Compliance Package 추가
- **보안**: PIN Lock, Setup Wizard, System Reset 추가
...
**마지막 업데이트**: 2026년 1월 (Share Capital, Real-time P&L, Asset Management, Audit Trail, Tax Provision, Period Management 완료 후)  
**다음 리뷰**: FBT 마감일 계산 및 예산 관리 시스템 개발 시작 전
