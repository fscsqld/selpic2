/**
 * Employee Add Form - 직원 추가 전용 폼
 * 
 * 모든 직원 정보를 한 번에 입력할 수 있는 종합 폼
 */

'use client'

import { useState, useEffect } from 'react'
import { Save, X, User, Mail, Phone, MapPin, Calendar, DollarSign, FileText, Building2, CreditCard } from 'lucide-react'
import { Employee, EmployeeType, PayFrequency } from '@/src/shared/types/employee'
import { indexedDBStorage } from '@/lib/storage/indexed-db'
import { setEmployeePassword } from '@/lib/auth/employee-auth'
...
export function EmployeeAddForm({ onSave, onCancel }: EmployeeAddFormProps) {
  ...
  useEffect(() => {
    if (formData.type === 'contractor') {
      // Contractor인 경우: ID를 CON-2026-XXX 형식으로 자동 생성
      const currentYear = new Date().getFullYear()
      const randomSuffix = Math.floor(Math.random() * 1000).toString().padStart(3, '0')
      const contractorId = `CON-${currentYear}-${randomSuffix}`
      
      if (!formData.employeeId || !formData.employeeId.startsWith('CON-')) {
        setFormData(prev => ({ 
          ...prev, 
          employeeId: contractorId, // CON-2026-XXX 형식
          superannuationRate: 0,
          taxFileNumber: '', // Contractor는 TFN 없음
        }))
      }
    } else {
  ...
  const handleSave = async () => {
    ...
    try {
      const employee: Employee = {
        ...
        annualLeaveBalance: formData.annualLeaveBalance || 0,
        sickLeaveBalance: formData.sickLeaveBalance || 0,
      ...
      if (formData.password && formData.password.trim()) {
        await setEmployeePassword(employee.employeeId, formData.password)
      }
      ...
      <div className="space-y-8">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, annualLeaveBalance: parseFloat(e.target.value) || 0 })}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                step="0.5"
                min="0"
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, sickLeaveBalance: parseFloat(e.target.value) || 0 })}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                step="0.5"
                min="0"
              />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-xs text-gray-500 mt-1">
                Name of the Superannuation Fund where contributions are paid
              </p>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">
        ...
        <div className="flex gap-4 pt-4 border-t border-gray-200">
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="flex-1 px-6 py-3 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:bg-gray-400 transition-colors flex items-center justify-center gap-2 font-medium"
          >
